<div class="hero--section" markdown>

# Courses

Browse courses available for adoption and follow each course's page for repositories, hub, and license details.

</div>

## UC Berkeley Courses

Each course below follows the same [adoption workflow](../how-to-adopt/index.md); the
course page fills in the specific repositories, Canvas template, hub, license, and any
course-specific notes.

<div class="grid cards cards-2x2" markdown>

-   ![Data 6](../assets/data6_logo.png){ .card-logo } **[Data 6: Computational Thinking with Data Science and Society](data6.md)**

    ---

    Full adoption package. Interdisciplinary introduction with course notes and grading infrastructure.

-   ![Data 8](../assets/data8logo.png){ .card-logo } **[Data 8: Foundations of Data Science](data8/adoption.md)**

    ---

    Full adoption package. Inference, computing, and statistics for first-year students.
    Two adoption versions; split textbook / materials licensing.

-   ![Data 100](../assets/data100_logo.png){ .card-logo } **[Data 100: Principles and Techniques of Data Science](data100.md)**

    ---

    Adoption materials in progress. Existing resources available now.

</div>

---

## Partner Courses

Courses developed outside UC Berkeley that share the same open-source infrastructure and
adoption workflow.

<div class="grid cards cards-2x2" markdown>

-   ![El Camino College](../assets/ecc.jpeg){ .card-logo } **[El Camino College CSCI 9: Practical Data Science](csci9.md)**

    ---

    *Not a UC Berkeley course.* A collaboration with UC Berkeley Data Science Modules.
    Adoption materials in progress. Existing resources available now.

</div>
